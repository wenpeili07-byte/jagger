...
but replace the permanently visible image strip with a square image preview
that belongs to each row and appears only while that row is active.
...
English text, description, link, arrow, and image asset.
...
part of the normal document flow and appear below their row text at a square
ratio. This preserves access to all service imagery on touch devices without
introducing a second tap before navigation.
...
The Services HTML should only change if a semantic or accessibility correction
is necessary. The homepage, Cases page, case-detail pages, global header,
typography, written content, and service destinations are outside this change.

## Acceptance Criteria

1. The deployed Services structure remains recognizable and unchanged in
   content and order.
2. No service image is permanently visible on desktop.
3. Hovering or focusing a row reveals only that row's correct image.
...
7. Mobile shows a readable square image in normal flow with no overflow.
