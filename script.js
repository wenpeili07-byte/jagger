...
function previewCaseBackground(index) {
  const card = cards[index];
  const scene = card ? getComputedStyle(card).getPropertyValue("--scene").trim() : "";
...
function resetCaseBackground() {
  if (shell) {
    shell.style.removeProperty("--active-case-scene");
  }
}
